...
pthread_cond_t conso[2] = {PTHREAD_COND_INITIALIZER};
pthread_mutex_t mutex = PTHREAD_MUTEX_INITIALIZER;


int nbCasesPleines, typeAttendu;
...
void deposer (TypeMessage leMessage, int rangProd) {
	...
	pthread_cond_signal(&conso[leMessage.type]);
...
void retirer (TypeMessage *unMessage, int rangConso) {
	pthread_mutex_lock(&mutex);
	if (nbCasesPleines == 0 || resCritiques.buffer[rangConso].type != typeAttendu)
		pthread_cond_wait(&conso[(typeAttendu+1)%2], &mutex);
	...
	if (nbCasesPleines >0)
		pthread_cond_signal(&conso[resCritiques.buffer[rangConso+1].type]);
