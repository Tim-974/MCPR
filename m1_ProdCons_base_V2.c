...
#define NB_FOIS_PROD   5 //10 
#define NB_FOIS_CONSO  5 //10 
...
pthread_cond_t prod[2] = {PTHREAD_COND_INITIALIZER};
pthread_cond_t conso = PTHREAD_COND_INITIALIZER;
pthread_mutex_t mutex = PTHREAD_MUTEX_INITIALIZER;


int nbCasesPleines, typeDerMsg;
...
void deposer (TypeMessage leMessage, int rangProd) {
	...
	if (nbCasesPleines == nbCases || leMessage.type == typeDerMsg)
		pthread_cond_wait(&prod[leMessage.type], &mutex);
		
	
	depot(&leMessage);
	nbCasesPleines++;
	typeDerMsg = leMessage.type;
	
	printf("\tProd %d : Message deposer = [T%d] %s (de %d)\n", 
         rangProd, leMessage.type, leMessage.info, leMessage.rangProd);
         
	pthread_cond_signal(&conso);
	if (nbCasesPleines != nbCases)
		pthread_cond_signal(&prod[((typeDerMsg+1)%2)]);
...
int main(int argc, char *argv[]) {
  ...
  if (argc <= 5) {
    printf ("Usage: %s <Nb Prod <= %d> <Nb Conso <= %d> <Nb Cases <= %d> <Nb fois message produit <= %d> <Nb fois message conso <= %d> \n", 
             argv[0], NB_PROD_MAX, NB_CONSO_MAX, NB_CASES_MAX, NB_FOIS_PROD, NB_FOIS_CONSO);
    exit(2);
  }
  ...
  if (nbFoisProd > NB_FOIS_PROD)
	nbFoisProd = NB_FOIS_PROD;
  nbFoisConso = atoi (argv[5]);
  if (nbFoisConso > NB_FOIS_CONSO)
	nbFoisConso = NB_FOIS_CONSO;
