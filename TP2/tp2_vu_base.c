...
pthread_cond_t sens[2] = {PTHREAD_COND_INITIALIZER};
...
void demanderAccesVU (int monSens) {
	...
	if ((nbVehicule != 0) && (monSens != sensCourant)){
		pthread_cond_wait(&sens[monSens], &mutex);
	}
	...
	pthread_cond_signal(&sens[monSens]);
...
void libererAccesVU (void) {
	...
	if (nbVehicule == 0){
		sensCourant = oppose(sensCourant);
		pthread_cond_signal(&sens[sensCourant]);
	}
